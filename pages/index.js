...
export default function Home() {
  ...
  const navStyle = "2xl:mx-2 md:text-xl text-lg h-20 2xl:p-7 xl:p-6 lg:p-5 md-p4 sm:p3 p-1 my-2 hover:font-bold";
      ...
      <div className="h-screen bg-black">
        <nav className="absolute flex w-full md:justify-end z-20 h-24 text-white justify-center">
          <div className={styles.main_list}>
            <ul className="flex  whitespace-nowrap md:mr-16">
              <li
                onMouseEnter={() => {
        ...
        <main>
          ...
          >
            {state.map((item, idx) => (
              <div key={idx} className="relative">
                <div className="absolute h-full md:items-start items-center justify-center flex gap-8 flex-col text-start ml-10 text-white z-10">
                  <h1 className="font-stargaze text-2xl text-center sm:text-3xl lg:text-4xl xl:text-5xl [text-shadow:_4px_0_rgb(0_0_0_/_50%)] ">
                    {item.original_title}
                  </h1>
                  <p className="font-titillium text-md xl:text-lg w-3/4 [text-shadow:_4px_0_rgb(0_0_0_/_40%)] ">
                    {item.overview}
                  </p>
                  ...
                  {/* <Link href={`https://hd.fmoviesto.site/search/?q=${item.title}`}>
                        ...
                        <Link href={`https://hd.fmoviesto.site/search/?q=${item.title}`}>
                          <span className=" text-zinc-600 bg-white p-3"> Tell Me More </span>
                        </Link> */}
                </div>
                <img
                  src={
          ...
          {/* <h3 className="text-xl text-white text-center border-b-4 rounded-lg border-indigo-500 border-double"></h3>
            ...
            {/* <div className="bg-gradient-to-r from-slate-800 to-purple-900  h-screen"></div> */}
        </main>
      </div>
    </>
  );
